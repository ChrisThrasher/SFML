...
int main()
{
    ...
    {
        const auto [it, success] = texts.emplace("ID", JoystickObject{{font, "<Not Connected>"}, {font}});
    ...
    {
        const auto [it,
                    success] = texts.emplace(axislabels[i],
                                             JoystickObject{{font, std::string(axislabels[i]) + ":"}, {font, "N/A"}});
        auto& [label, value] = it->second;
    ...
    for (unsigned int i = 0; i < sf::Joystick::ButtonCount; ++i)
    {
        sstr.str("");
        sstr << "Button " << i;
        const auto [it, success] = texts.emplace(sstr.str(), JoystickObject{{font, sstr.str() + ":"}, {font, "N/A"}});
        auto& [label, value]     = it->second;
